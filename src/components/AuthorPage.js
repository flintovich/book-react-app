...
export default class App extends Component {
  ...
  getAuthorData(authorID) {
    const authorData = this.getAuthorList();
    let author = null;
    for(let i in authorData){
      if(authorData[i].id === authorID){
        author = authorData[i];
        break;
      }
    }
  ...
  render() {
    const authorData = this.getAuthorData(this.props.routeParams.authorId);
    return (
      <div>
        <article>
          <h1>{authorData.name}</h1>
          <blockquote>{authorData.biography}</blockquote>
        </article>
